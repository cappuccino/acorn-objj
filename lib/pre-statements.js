...
var acorn = require("acorn"),
    ...
    Macro = require("./pre-macro"),
    Preprocessor = require("./preprocessor"),
    PreprocessorToken = require("./pre-tokenize").PreprocessorToken;
...
acorn.Parser.prototype.objj_parseDefine = function()
{
    ...
    pp.addMacro(new Macro(name, nameStart, nameEnd, parameters, parameterMap, isFunction, isVariadic, tokens));
};
...
acorn.Parser.prototype.objj_parseMacroArguments = function(macro, context)
{
    ...
    while (true)
    {
        switch (this.type)
        {
            ...
            case tt.eof:
                this.raise(this.lastTokEnd, "Unexpected EOF in macro call");
                break;
...
acorn.Parser.prototype.objj_expectedPreEndif = function(pos, ifState, saw)
{
    this.raise(
        this.pos,
...
acorn.Parser.prototype.objj_parsePreIf = function()
{
    ...
    if (type === tt._objj_pre_elif)
    {
        ...
        if (state.phase === Preprocessor.ifState.afterElse)
            this.objj_expectedPreEndif(startPos, state, "elif");
...
acorn.Parser.prototype.objj_parsePreElse = function()
{
    ...
    if (this.type !== tt.objj_eol)
        this.raise(startPos, "#else must be followed by the token EOL");
    ...
    if (pp.ifStack.length > 0)
    {
        ...
        if (state.phase === Preprocessor.ifState.afterElse)
            this.objj_expectedPreEndif(startPos, state, "else");

        state.phase = Preprocessor.ifState.afterElse;
        pp.skipping = state.skipping = !state.skipping;
    }
    else
        this.raise(startPos, "#else without matching #if");
};
...
acorn.Parser.prototype.objj_parsePreEndif = function()
{
    ...
    if (this.type !== tt.objj_eol)
        this.raise(startPos, "#endif must be followed by the token EOL");
...
{
    var startPos = this.start,
        expr = this.objj_parsePreprocessExpression();

    if (this.type !== tt.objj_eol)
        this.raise(startPos, "#" + type.keyword + " expressions must be followed by EOL");

    var message = String(this.objj_evalPreprocessExpression(expr));

    if (type === tt._objj_pre_error)
        this.raise(startPos, message);
    else
        issueHandler.addWarning(
            this.objj.issues,
            this.input,
            this.objj.file,
            new acorn.SourceLocation(this, startPos, startPos),
            message
        );
};
