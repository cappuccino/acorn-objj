...
var acorn = require("acorn"),
    ...
    macros = require("./pre-macro.js"),
    preprocessor = require("./preprocessor.js"),
    PreprocessorToken = require("./pre-tokenize.js").PreprocessorToken;

// jscs: disable requireMultipleVarDecl

var PreprocessorIfState = preprocessor.PreprocessorIfState,
    PreprocessorState = preprocessor.PreprocessorState,
    tt = acorn.tokTypes;
...
acorn.Parser.prototype.objj_parsePreprocess = function()
{
    ...
    pp.state &= ~PreprocessorState.expandMacros;
    pp.state |= PreprocessorState.directive;
    this.next();
    ...
    switch (directive)
    {
        ...
        case tt._objj_pre_undef:
            ...
            else
            {
                ...
                this.expect(tt.name, "!Macro name missing");
    ...
    pp.state = PreprocessorState.default;
...
acorn.Parser.prototype.objj_parseMacroParameters = function(info, parameters, parameterMap)
{
    ...
    while (this.type !== tt.parenR)
    {
        if (expectComma === true)
        {
            this.expect(tt.comma, "between macro parameters");
            expectComma = false;
        }
        else
        {
            ...
            switch (this.type)
            {
                case tt.name:
                    ...
                    else
                    {
                        parameter = {
                            name: argName,
                            start: this.lastTokStart, // lastTokStart/End because we did next() above
                            end: this.lastTokEnd,
                            variadic: false
                        };
                ...
                case tt.ellipsis:
                    ...
                    parameter = {
                        ...
                        start: namedVariadicArg ? this.lastTokStart : this.start,
                        end: namedVariadicArg ? this.lastTokEnd : this.end,
                        variadic: true
                    };
...
acorn.Parser.prototype.objj_readMacroBodyTokens = function(
    parameters,
    parameterMap,
    tokens,
    isVariadic,
...
)
{
    ...
    while (this.type !== tt.objj_eol && this.type !== tt.eof)
    {
        ...
        switch (this.type)
        {
            ...
            case tt.objj_stringifiedName:
                ...
                // Tell the body token what parameter it belongs to, if any
                token.macroParameter = parameterMap[this.value];

                // If it's a stringified name, make sure the name is a valid macro parameter
                if (this.type === tt.objj_stringifiedName && token.macroParameter === undefined)
                    this.raise(this.start, "# is not followed by a macro parameter");

                expandArgs = expandArgs || token.macroParameter !== undefined;
                break;

            case tt.objj_preTokenPaste:
                if (tokens.length === 0)
                    this.raise(tokens.start, "'##' cannot be at the beginning of a macro expansion");
    ...
    if (tokens.length > 0 && tokens.last().type === tt.objj_preTokenPaste)
...
acorn.Parser.prototype.objj_parseDefine = function()
{
    ...
    var nameToken = new PreprocessorToken(this.objj.preprocessor);

    this.expect(tt.name, "!Macro name missing");

    if (nameToken.value === "__VA_ARGS__")
        this.raise(nameToken.start, "__VA_ARGS__ may only be used within the body of a variadic macro");
    else if (nameToken.value === "defined")
        this.raise(nameToken.start, "'defined' may not be used as a macro name");

    var parameters = [],
        parameterMap = Object.create(null), // Don't inherit from Object so we don't have to worry about name collisions
        isFunction = false,
        isVariadic = false,
        variadicParameterName = "__VA_ARGS__";

    // '(' Must follow directly after identifier to be a valid macro with parameters
    if (this.input.charCodeAt(this.lastTokEnd) === 40) // '('
    {
        var info = {
    ...
    var tokens = [],
        expandArgs = this.objj_readMacroBodyTokens(
            parameters,
            parameterMap,
            tokens,
            isVariadic,
...
/**
 * Parse arguments passed to a macro.
 *
 * @param macro
 * @returns {MacroArgument[]}
 */
acorn.Parser.prototype.objj_parseMacroArguments = function(macro)
{
    // Eat the '('
    this.next();

    var arg = new preprocessor.MacroArgument(macro, this),
    ...
    while (true)
    {
        switch (this.type)
        {
            ...
            case tt.parenR:
                if (--parenLevel === 0)
                {
                    ...
                    if (this.objj.preprocessor.macroExpansionStack.length === 1)
                    {
                        this.next();
                    }
            ...
            case tt.comma:
                ...
                if (parenLevel === 1)
                {
                    ...
                    if (macro.isVariadic === false && args.length > macro.parameters.length)
                        break scanArguments;

                    arg = new preprocessor.MacroArgument(macro, this);
        ...
        var pp = this.objj.preprocessor,
            token = new PreprocessorToken(pp);
...
{
    var error = issueHandler.addError(
        this.objj.issues,
        this.input,
        this.objj.file,
...
acorn.Parser.prototype.objj_parsePreIf = function()
{
    ...
    if (type === tt._objj_pre_elif)
    {
        ...
        // If we already saw an #else, #elif is not valid
        if (state.phase === PreprocessorIfState.afterElse)
            this.objj_expectedPreEndif(this.start, this.end, state, "elif");

        // If this is a nested #if statement and the parent was skipping,
        // don't change the skipping state.
        if (pp.ifStack.length < 2 || pp.ifStack[pp.ifStack.length - 2].skipping === false)
        {
            // If one of the previous tests has been selected, we skip the rest
            // of the clauses. Otherwise we wait until we evaluate the expression.
            pp.skipping = state.skipping = state.selected;
        }
    }
    else // if/ifdef/ifndef
    {
        // Push some state on the stack
        state = {
            // We want to know what kind of statement we are in
            type: type,

            // For error reporting
            input: this.input,

            // Where did this statement start
            pos: startPos,

            // So we know whether whether #elif or #else is valid
            phase: PreprocessorIfState.afterIf,

            // This flag indicates whether one of the tests in
            // an if/elif/else statement has evaluated true
            selected: false,

            // Whether we are skipping the current clause.
            // For now we assume it's the same as the global state.
            // If we are not skipping globally, the state will
            // be determined by evaluating the expression below.
            skipping: pp.skipping
        };
...
acorn.Parser.prototype.objj_parsePreElse = function()
{
    ...
    if (pp.ifStack.length > 0)
    {
        ...
        // If an #else was already seen, it's an error
        if (state.phase === PreprocessorIfState.afterElse)
            this.objj_expectedPreEndif(this.lastTokStart, this.lastTokEnd, state, "else");

        state.phase = PreprocessorIfState.afterElse;

        // If this is a nested #if and we are skipping, don't change the skipping state
        if (pp.skipping === false || pp.ifStack.length === 1)
        {
            // If a previous clause was selected, we skip this clause.
            // Otherwise this clause is selected.
            pp.skipping = state.skipping = state.selected;
            state.selected = true;
        }
    }
...
acorn.Parser.prototype.objj_parsePreEndif = function()
{
    ...
    if (pp.ifStack.length > 0)
    {
        ...
        // Resume the skipping state of the next #if up the stack.
        // If the stack is empty, we are no longer skipping.
        if (pp.ifStack.length > 0)
            pp.skipping = pp.ifStack.last().skipping;
        else
            pp.skipping = false;
    }
    else
