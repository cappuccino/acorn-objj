...
var acorn = require("acorn"),
    PreprocessorState = require("./preprocessor.js").PreprocessorState,
...
/**
 * Captures the complete state of the parser in a token.
 *
 * @class
 * @param {Preprocessor} preprocessor
 * @param {acorn.Parser} parser
 */
var PreprocessorToken = function(preprocessor, parser)
{
    parser = parser || preprocessor.parser;

    this.input = parser.input;
    this.sourceFile = parser.sourceFile;
    ...
    if (parser.options.locations === true)
    {
...
exports.PreprocessorToken = PreprocessorToken;

exports.init = function()
{
    acorn.Parser.prototype.objj_readWord = objj_readWord;
};
...
acorn.Parser.prototype.objj_skipSpace = function()
{
    ...
    while (this.pos < this.input.length)
    {
        ...
        switch (ch)
        {
            ...
            case 13:
                if ((pp.state & PreprocessorState.directive) !== 0)
                    return;
            ...
            case 8233:
                ...
                if (this.options.locations === true)
                {
                    ++this.curLine;
...
acorn.Parser.prototype.objj_skipToEOL = function(dontTokenize)
{
    if (dontTokenize === true)
    {
        // Scan for EOL not preceded by \
        while (true)
        {
            eolRegex.lastIndex = this.pos;

            var match = eolRegex.exec(this.input);
...
/**
 * Returns the preprocessor token type for the string at the current position,
 * starting with the given character code. If no preprocessor token is at the current position,
 * null is returned.
 *
 * @param {number} code - The character code of the character at the current position.
 * @returns {?acorn.TokenType}
 */
...
function objj_readWord()
{
    var word = this.readWord1(),
        pp = this.objj.preprocessor,
        type = tt.name;

    if ((this.options.ecmaVersion >= 6 || this.containsEsc === false))
    {
        if (this.type === tt._objj_preprocess && this.objj_isPreprocessorKeyword(word) === true)
        {
            pp.state |= PreprocessorState.directive;

            return this.finishToken(preprocessorKeywordMap[word], word);
        }
        else if (pp.skipping === false && (pp.state & PreprocessorState.expandMacros) !== 0)
        {
            var macro = pp.getMacro(word);

            if (macro !== undefined)
            {
                // If the macro was expanded, then we have to read from the stream
                // and load up the first token in the stream. If it wasn't expanded,
                // return the macro name.
                this.finishToken(tt.name, word);

                if (pp.expandMacro(macro, pp.stream) === true)
                {
                    pp.readFromStream();
        ...
        else if (this.objj.options.objj === true && this.objj_isObjjKeyword(word) === true)
...
acorn.Parser.prototype.objj_setToken = function(token)
{
    ...
    this.sourceFile = token.sourceFile;
