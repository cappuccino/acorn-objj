...
var getVersionString = exports.getVersionString = function()
{
    var env = getEnvironment();

    return util.format("%s v%s (acorn v%s)", env.executable, env.version, env.acornVersion);
};
...
var optionator,
    ...
    optionatorConfig = {
        prepend: chalk.cyan(getVersionString() + " \nUsage") +
...
{
    return utils.colorizeOptions(optionator.generateHelp({ interpolate: getEnvironment() }));
}
...
function parseOptions(args, parseOpts)
{
    ...
    if (files.length === 0 && !options.useStdin)
...
{
    acorn.Parser.prototype.raise(offset, "Expected a semicolon");
}
    ...
    try
    {
        ...
        else
        {
            if (options.useStdin)
            {
                ...
                source = require("fs").readFileSync(options.file);
