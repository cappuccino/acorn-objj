...
var optionator,
    optionDefinitions = [
        ...
        },
        {
            ...
            alias: "p",
            type: "String",
            description: "Macro definitions are read from the file at this path."
        },
        {
            option: "color",
            type: "Boolean",
...
exports.run = function(args, runOptions)
{
    ...
    catch (ex)
    {
        if (!options.silent)
        {
            ...
            // Check to see if it's an acorn error
            if (ex instanceof SyntaxError && ex.loc && source)
            {
                error = issueHandler.addAcornError(issues, ex, source, options.file);
            }
            else if (!(ex instanceof issueHandler.Error))
            {
    ...
    if (!options.silent)
    {
        if (issues.length > 0)
        {
            ...
            issueHandler.logIssues(issues, options.color);
        }

        if (options.stackTrace && error)
        {
            var trace = error.getStackTrace ?
                        error.getStackTrace() /* istanbul ignore next: only occurs if there is a bug in my code */:
                        error.stack;
