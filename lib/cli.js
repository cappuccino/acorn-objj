...
function parseOptions(args, parseOpts)
{
    ...
    if (options.version === true || options.help === true)
        return options;
    ...
    if (files.length === 0 && useStdin === false)
...
exports.run = function(args, runOptions)
{
    ...
    catch (ex)
    {
        if (options.silent !== true)
        {
            ...
            if ((ex instanceof issueHandler.Issue) === false)
            {
    ...
    if (options.silent !== true)
    {
        ...
        if (error !== undefined && options.stackTrace === true)
        {
            var filter = ["Parser\\.acorn\\.Parser\\.objj_raise", "Preprocessor\\.captureTokenExpansion"];
