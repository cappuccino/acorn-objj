    ...
    catch (ex)
    {
        if (!options.silent)
        {
            ...
            if (ex instanceof SyntaxError && ex.loc && source)
                issueHandler.addAcornError(issues, ex, source, options.file);
            else if (!(ex instanceof issueHandler.AbortError))
                console.log(chalk.red("error: ") + chalk.grey(ex.message));
        }

        exitCode = 1;
    }

    if (!options.silent && issues.length > 0)
    {
