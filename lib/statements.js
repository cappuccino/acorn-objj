...
{
    if (this.objj_getState("@implementation") === true || this.objj_getState("@interface") === true)
    ...
    if (this.eat(tt.colon) === true)
        objj.superclass = this.parseIdent(true);
    else if (this.eat(tt.parenL) === true)
    {
...
acorn.Parser.prototype.objj_parseIvarDeclaration = function()
{
    ...
    if (this.strict === true && this.isReservedWord(type.objj.name) === true)
    ...
    if (this.strict === true && this.isReservedWord(objj.id.name) === true)
        this.raise(objj.id.start, "Binding '" + objj.id.name + "' in strict mode");

    if (this.eat(tt._objj_accessors) === true)
...
acorn.Parser.prototype.objj_parseAccessors = function(objj)
{
    ...
    if (this.eat(tt.parenL) === false || this.eat(tt.parenR) === true)
        return;
...
acorn.Parser.prototype.objj_parseMethodDeclaration = function(node)
{
    ...
    if (this.eat(tt.parenL) === true)
    {
        var typeNode = this.startNode();

        if (this.eat(tt._objj_action) === true || this.eat(tt._objj_IBAction) === true)
        {
            objj.action = this.finishNode(typeNode, "objj_ActionType");
            typeNode = this.startNode();
        }

        if (this.eat(tt.parenR) === false)
        {
    ...
    while (true)
    {
        if (this.type !== tt.colon)
        {
            ...
            if (first === true && this.type !== tt.colon)
                break;
        }
        else
            selectors.push(null);
...
acorn.Parser.prototype.objj_parseProtocolDeclaration = function()
{
    ...
    while (!this.eat(tt._objj_end))
    {
        ...
        if (this.eat(tt._objj_required) === true)
            continue;

        if (this.eat(tt._objj_optional) === true)
        {
            while (this.eat(tt._objj_required) === false && this.type !== tt._objj_end)
