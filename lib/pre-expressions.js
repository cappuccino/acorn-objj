...
var PreprocessorState = preprocessor.PreprocessorState,
    tt = acorn.tokTypes;
...
acorn.Parser.prototype.objj_preprocessParseExprOp = function(left, minPrec)
{
    ...
    if (this.type.prefix === true || this.type.binop !== null)
    {
...
{
    if (this.objj_isPreprocessorToken(this.type) === true && this.type.prefix === true)
    {
...
acorn.Parser.prototype.objj_preprocessParseExprAtom = function()
{
    ...
    switch (this.type)
    {
        ...
        case tt.num:
            if (/^\d+$/.test(this.input.substring(this.start, this.end)) === true)
        ...
        case tt.parenL:
            ...
            node.start = start;
            node.end = this.end;
            this.expect(tt.parenR, "in preprocessor expression");
            break;

        default:
            if (this.type.keyword !== undefined)
            {
...
acorn.Parser.prototype.objj_preprocessParseDefined = function(node)
{
    ...
    if (haveParens === true)
        this.expect(tt.parenR, "after macro name");
