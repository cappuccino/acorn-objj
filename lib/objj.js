...
{
    if (this.eat(type) === false)
    {
        var error;

        if (context !== undefined && context.charAt(0) === "!")
            error = context.substr(1);
        else
        {
            error = "Expected '" + type.label + "'";
            error += context === undefined ? "" : " " + context;
        }
...
{
    if (this.eat(tt.semi) === false && this.insertSemicolon() !== true)
...
acorn.Parser.prototype.objj_raise = function(next, pos, message)
{
    ...
    catch (ex)
    {
        ...
        {
            if (this.objj.preprocessor !== null)
