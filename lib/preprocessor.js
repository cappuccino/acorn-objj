        ...
        */
        ...
        if (!same)
        {
            issueHandler.addWarning(
                ...
                new acorn.SourceLocation(this.parser, macro.start, macro.end),
            ...
            if (this.lookupMacroParameter(macro, token))
            {
                ...
                if (token.type === tt.name)
                {
                    if (token.macroParameter.variadic)
                        this.expandVariadicParameters(macro, args, token, bodyTokens, argTokens);
                    else
...
var PreprocessorToken = require("./pre-tokenize").PreprocessorToken;

require("./pre-parser");
require("./pre-statements");
