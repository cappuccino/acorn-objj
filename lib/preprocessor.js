...
Preprocessor.prototype.pushMacro = function(macro, nameToken, context)
{
    var state;
...
Preprocessor.prototype.expandMacro = function(macro, expandedTokens, context)
{
    ...
    this.pushMacro(macro, nameToken, context);
    parser.next();
    ...
    if (!nested)
    {
        if (isMacroCall)
        {
            ...
            parser.lastTokStart = stateBefore.lastTokStart;
            parser.lastTokEnd = stateBefore.lastTokEnd;
...
Preprocessor.prototype.pasteTokens = function(macro, args, leftToken, rightToken, index, expandedTokens)
{
    ...
    */
         ...
         ++i)
    {
        ...
        {
            leftToken = tokensToPaste[i];

            var tokenText = leftToken === null ? "" : leftToken.input.substring(leftToken.start, leftToken.end);

            // Move to the token after ##
            i += 2;

            // Paste its text if it wasn't an empty arg
            rightToken = tokensToPaste[i];

            if (rightToken !== null)
                tokenText += rightToken.input.substring(rightToken.start, rightToken.end);

            var pastedToken = null;

            if (tokenText.length > 0)
                pastedToken = this.parsePastedTokens(tokenText);

            if (pastedToken)
                expandedTokens.push(pastedToken);
            else
            {
                if (leftToken)
...
Preprocessor.prototype.parsePastedTokens = function(tokenText)
{
    var parser = this.parser,
        parsedToken = null;
    ...
    if (parsedToken === null)
    {
        var macro = this.macroStack[0];

        // Add an error pointing to the macro call
