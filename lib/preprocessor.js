...
Preprocessor.prototype.expandMacroFirstPass = function(macro, args, expandedTokens)
{
    ...
    for (var i = 0, lastPasteIndex = macro.tokens.length - 3; i < macro.tokens.length; ++i)
    {
        ...
        {
            var lastPastedIndex = this.pasteTokens(macro, args, expandedTokens, i);

            // Since the pasted tokens replaced the existing ones, back up one
            // so the next time through the loop will start at the last pasted token.
            i = lastPastedIndex - 1;
            continue;
        }
...
Preprocessor.prototype.pasteTokens = function(macro, args, expandedTokens, index)
{
    /*
    ...
    */
    ...
    this.expandPasteToken(macro, args, macro.tokens[lastIndex], tokensToPaste);
         ...
         ++i)
    {
        ...
        if (i < lastPasteIndex && tokensToPaste[i + 1].type === tt.objj_preTokenPaste)
        {
            ...
            if (tokenText.length > 0)
                pastedToken = this.parsePastedTokens(tokenText, pasteToken);
...
Preprocessor.prototype.parsePastedTokens = function(tokenText, pasteToken)
{
