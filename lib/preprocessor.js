...
/**
 * @enum {number} MacroExpansionPhase
 * @constant
 */
var MacroExpansionPhase = {
 ...
 * @property {MacroExpansionPhase} phase - Which phase of expansion generated the call.
...
/**
 * @typedef {object} MacroCallState
 *
 * @property {Macro} macro - The macro that is being called.
 * @property {?Macro} caller - The macro that called this macro, or null if none.
 * @property {MacroExpansionPhase} phase - The phase of expansion in which the call occurred.
 ...
 * @property {boolean} readFromSource - Whether we are reading from source when the macro was called.
 * @property {PreprocessorToken[]} tokenStream - For nested macro calls, the token stream being parsed.
 * @property {number} tokenStreamIndex - For nested macro calls, the index of the current token.
 */

/**
 * Push a macro call on the stack.
 *
 * @param {Macro} macro
 * @param {MacroCallContext} context
 * @returns {MacroCallState}
 */
Preprocessor.prototype.pushMacro = function(macro, context)
{
    var parser = this.parser,
        token = context ? context.tokens[context.tokenIndex] : parser,
        state = {
            macro: macro,
            caller: context ? context.caller : null,
            phase: context ? context.phase : MacroExpansionPhase.call,
            input: token.input,
            start: token.start,
            end: token.end,
    ...
    if (context)
    {
        ...
        this.tokenStreamIndex = context.tokenIndex + 1;
    }
    else
    {
...
Preprocessor.prototype.defineMacro = function(definition)
{
    ...
    this.state |= Preprocessor.state.directive;
...
Preprocessor.prototype.expandMacro = function(macro, expandedTokens, context)
{
    ...
    var parser = this.parser,
        oldExpand = this.state & Preprocessor.state.expandMacros;

    this.state &= ~Preprocessor.state.expandMacros;

    // Save the macro name as a token in case it is a function macro which has no arguments
    parser.finishToken(tt.name, macro.name);
    ...
    var nested = this.macroStack.length > 1,
        isMacroCall = true,
        args = null,
        tokenAfterMacro;
    ...
    // If the macro is not nested and will be expanded, save some state.
    if (!nested)
    {
        if (isMacroCall)
        {
    ...
    {
        // If the macro is nested and has no args and we have not reached the end of
    ...
    if (!nested)
    {
        ...
        {
            parser.objj_setToken(nameToken);
            this.skipSpace();
        }
    }
        ...
        for (var i = 0; i < arg.tokens.length; ++i)
        {
            ...
            if (token.type === tt.name)
            {
                ...
                if (nestedMacro)
                {
                    var context = {
                            caller: macro,
                            phase: MacroExpansionPhase.args,
                            tokens: arg.tokens,
                            tokenIndex: i
                        };
...
Preprocessor.prototype.expandVariadicParameters = function(macro, args, token, bodyTokens, argTokens)
{
    ...
    else
    {
        ...
        {
            Array.prototype.push.apply(argTokens, this.expandMacroArgument(macro, args[i]));
...
Preprocessor.prototype.expandMacroBody = function(macro, args, expandedTokens)
{
    ...
    this.expandMacroSecondPass(macro, bodyTokens, expandedTokens);
};
...
Preprocessor.prototype.expandMacroFirstPass = function(macro, args, expandedTokens)
{
    ...
    for (var i = 0, lastPasteIndex = macro.tokens.length - 3; i < macro.tokens.length; ++i)
    {
        ...
        if (expandArgs && this.lookupMacroParameter(macro, token))
        {
            var argTokens = [];
    ...
    for (var i = 0; i < bodyTokens.length; ++i)
    {
        ...
        if (token.type === tt.name)
        {
            ...
            if (nestedMacro)
            {
                var context = {
                        caller: macro,
                        phase: MacroExpansionPhase.body,
                        tokens: bodyTokens,
                        tokenIndex: i
                    };
...
Preprocessor.prototype.pasteTokens = function(macro, args, leftToken, rightToken, index, expandedTokens)
{
    ...
    */
         ...
         ++i)
    {
        ...
        {
            this.parsePastedTokens(tokensToPaste, i, expandedTokens);

            // Move to the right token
            i += 2;
        }
        else
            expandedTokens.push(tokensToPaste[i]);
    }
...
Preprocessor.prototype.expandPasteToken = function(macro, args, token, expandedTokens)
{
    if (this.lookupMacroParameter(macro, token))
    {
        ...
        if (arg.tokens.length !== 0)
        {
            ...
            if (token.type === tt.name)
                Array.prototype.push.apply(expandedTokens, arg.tokens);
            else // type === _stringifiedName
                expandedTokens.push(this.stringifyMacroArgument(arg));
        }
        else
...
Preprocessor.prototype.captureMacroStack = function(parser, errorMessage)
{
    var first = true,
        stackLength = this.macroStack.length;

    for (var i = 0; i < stackLength; ++i)
    {
        var state = this.macroStack[i],
            adder,
            message;
        ...
        if (first)
        {
            ...
            if (i < stackLength - 1)
            {
                var nextState = this.macroStack[i + 1];

                if (nextState.phase === MacroExpansionPhase.args && nextState.caller === state.macro)
                    continue;
            }
        ...
        else
        {
            ...
            message = format("expanded from macro '%s'", state.caller.name);
        }
...
Preprocessor.prototype.parsePastedTokens = function(tokens, index, expandedTokens)
{
    ...
    if (parsedToken === null)
    {
        ...
        if (leftToken)
            expandedTokens.push(leftToken);

        // istanbul ignore else
        if (rightToken)
            expandedTokens.push(rightToken);

        this.captureMacroStack(parser, format("pasting formed '%s', an invalid token", tokenText));

        var state = this.macroStack.last();
...
Preprocessor.prototype.readFromSource = function()
{
    ...
    this.nextToken = next;
    this.skipSpace = acorn.Parser.prototype.objj_sourceSkipSpace.bind(parser);
...
Preprocessor.prototype.streamNextToken = function()
{
    ...
    {
        var token = this.tokenStream[this.tokenStreamIndex++];
