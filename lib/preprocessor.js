...
{
    var file = this.parser.objj.file;

    this.parser.objj.file = source;

    for (var i = 0; i < definitions.length; ++i)
        this.defineMacro(definitions[i]);

    this.parser.objj.file = file;
};
...
Preprocessor.prototype.expandMacro = function(macro, expandedTokens, context)
{
    ...
    if (macro.isFunction)
    {
        ...
        if (this.parser.eat(tt.parenL))
            args = this.parser.objj_parseMacroArguments(macro);
        else
            isMacroCall = false;
    }
...
Preprocessor.prototype.parse = function(next)
{
    ...
    this.defineMacros(this.parser.objj.options.macros || /* istanbul ignore next */ [], "<command line>");
