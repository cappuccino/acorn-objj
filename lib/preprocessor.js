...
Preprocessor.prototype.expandMacroFirstPass = function(macro, args, expandedTokens)
{
    ...
    for (var i = 0, lastPasteIndex = macro.tokens.length - 3; i < macro.tokens.length; ++i)
    {
        ...
        {
            i = this.pasteTokenSeries(macro, args, expandedTokens, i);
            continue;
        }
...
Preprocessor.prototype.pasteTokenSeries = function(macro, args, expandedTokens, index)
{
    /*
        ...
        We return the index of the last token that we expanded/stringified.
    */
    var lastIndex = index,
        tokensToPaste = this.expandPasteToken(macro, args, macro.tokens[lastIndex]);
    ...
    do
    {
        ...
        // Move to the token following ##
        ++lastIndex;

        // Expand the next token
        tokensToPaste = tokensToPaste.concat(this.expandPasteToken(macro, args, macro.tokens[lastIndex]));

        // Move to the next token, stop if it isn't ##
        ++lastIndex;
    }
    while (lastIndex < macro.tokens.length && macro.tokens[lastIndex].type === tt.objj_preTokenPaste);

    /*
        Second pass. Go through the expanded tokens, pasting any
        series of single tokens separated by ##. The rest are
        added as is.
    */
    for (var i = 0, lastPasteIndex = tokensToPaste.length - 1;
         ...
         ++i)
    {
        // Is the next token ##? If so, paste a series of (## token)+
        // Continue doing so until the next token is not ##. A null token
        // means it was a macro parameter whose argument was empty.
        if (i < lastPasteIndex && tokensToPaste[i + 1].type === tt.objj_preTokenPaste)
        {
            var token = tokensToPaste[i],
                tokenText = token === null ? "" : token.input.substring(token.start, token.end);
            ...
            do
            {
                ...
                // Paste its text
                token = tokensToPaste[i];

                if (token !== null)
                    tokenText += token.input.substring(token.start, token.end);

                // Move to the next token and continue
                ++i;
            }
            while (i < tokensToPaste.length &&
                   tokensToPaste[i] &&
                   tokensToPaste[i].type === tt.objj_preTokenPaste);

            if (tokenText.length > 0)
                expandedTokens.push(this.parsePastedToken(tokenText, macro, token));

            if (i === tokensToPaste.length)
                break;
        }
    ...
    // i points just past the last expanded token, so we return i - 1
    return lastIndex - 1;
};

Preprocessor.prototype.expandPasteToken = function(macro, args, token)
{
    var expandedTokens = [];
...
Preprocessor.prototype.parsePastedToken = function(tokenText, macro, token)
{
    ...
    try
    {
        ...
        var tokenizer = acorn.tokenizer(tokenText, this.parser.options);
    ...
    if (parsedToken === null)
    {
        throw issueHandler.addFatalError(
            this.parser.objj.issues,
            this.parser.input,
            this.parser.objj.file,
            token,
